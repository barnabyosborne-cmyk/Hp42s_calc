...
| `tools/gen_ic_footprints.py` | The same for the regulator, the IR emitter, the status LED, the ESD array and the USB-C receptacle. |
...
- **The Alps tact switches' land pattern.** The Alps specification that turned
  up is text only: every dimension in it points at an assembly drawing that is
  not in the document. The two buttons are still on KiCad's footprint, which is
  the last part on the board not checked against its own maker's drawing.
