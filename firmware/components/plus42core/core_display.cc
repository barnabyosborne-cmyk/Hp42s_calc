...
static int ext_str_cat[] = {
    CMD_APPEND,  CMD_C_TO_N,    CMD_EXTEND, CMD_HEAD,   CMD_IP_TO_S, CMD_LENGTH,
    CMD_TO_LIST, CMD_FROM_LIST, CMD_LIST_T, CMD_LXASTO, CMD_NEWLIST, CMD_N_TO_C,
...
static int column_width(vartype *m, int4 imin, int4 imax, int4 j) {
    ...
    pixel_width = to_int(log10(j + 1.0)) * 4 + 3;
...
void redisplay(int mode) {
    ...
    if (menu_id == MENU_CATALOG) {
        ...
        if (varmenu_length == 0 && varmenu_eqn == NULL) {
            if (mode_appmenu == MENU_VARMENU)
                // Prevent infinite recursion, in cases where the varmenu
                // program has disappeared without the menu state having
    ...
    if (flags.f.prgm_mode) {
        ...
        if (m->type == TYPE_LIST) {
            ...
            int digits = to_int(log10(double(matedit_view_i + mrows))) + 1;
            for (int r = msg_lines; r < mrows; r++) {
                int rn = r + matedit_view_i + 1;
                int d = to_int(log10(double(rn))) + 1;
                int bufptr = 0;
            ...
            try {
                ...
                int header_width = 4 * (to_int(log10(double(matedit_view_i + mrows1))) + 1) + 1;
